...
import com.epam.winter_java_lab.entiities.order.ingredient.soup.Potato;
...
public class PotatoCommand extends IngredientCommand {

    public PotatoCommand(Cafe cafe, User user) {
        super(cafe, user);
    }

    @Override
    public void addIngredient(Order order, User user) {
        order = new Potato(order);
        user.setOrder(order);
    }
}
